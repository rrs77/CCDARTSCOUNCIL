...
export function Scene2() {
  ...
  return (
    ...
    >
      ...
      <motion.div 
        className="absolute top-[-20%] right-[-10%] w-[50vw] h-[50vw] rounded-full border border-white/5 bg-white/5 backdrop-blur-3xl"
        animate={{ rotate: 360, scale: [1, 1.05, 1] }}
        transition={{ duration: 30, repeat: Infinity, ease: 'linear' }}
      />

      <div className="relative z-10 w-full px-[6vw] flex items-center justify-between gap-12">
        <div className="w-[40%] text-white">
          <motion.div
            initial={{ opacity: 0, y: 20 }}
            animate={phase >= 1 ? { opacity: 1, y: 0 } : { opacity: 0, y: 20 }}
            transition={springs.snappy}
            className="mb-6 inline-block px-5 py-2 rounded-full bg-white/20 border border-white/30 font-bold tracking-wide text-[1.2vw] shadow-soft"
          >
            SAVE SUNDAYS
          </motion.div>
          <motion.h2 
            className="text-[4.5vw] font-black leading-[1.05] tracking-tight mb-6"
          ...
          <motion.p
            className="text-[1.8vw] text-white/90 max-w-lg leading-snug"
            initial={{ opacity: 0 }}
        ...
        <div className="w-[55%] h-[70vh] relative perspective-[1200px]">
          <motion.div

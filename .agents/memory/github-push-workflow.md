...
**How to apply:** Auth via the GitHub connection: fetch token from `https://$REPLIT_CONNECTORS_HOSTNAME/api/v2/connection?include_secrets=true` with `X_REPLIT_TOKEN: repl $REPL_IDENTITY` (note: `connector_names=github` filter with include_secrets returned empty — omit the filter). Match the connection by `id` containing "github" (items have `connector_id: None`); token at `settings.access_token`. Use GIT_ASKPASS with username `x-access-token` — must `export GIT_ASKPASS` (inline env prefix on the same line as a heredoc setup didn't take; unexported it prompts and the sandbox kills the command). Never print the token. If the side branch `workspace-main` already exists remotely, DELETE its ref via API first, then push. After a server-side merge, local main is still behind origin/main — the next push will again be non-fast-forward; repeat the side-branch + API merge approach.
